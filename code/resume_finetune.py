#!/usr/bin/env python3
"""Resume fine-tuning from restormer_ft_epoch5.pth in checkpoints/"""
...
START_EPOCH   = 6
NUM_EPOCHS    = 10
WEIGHT_FILE   = os.path.join(PROJECT_DIR, 'checkpoints', 'restormer_ft_epoch5.pth')
...
class SIDDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if h < self.crop_size or w < self.crop_size:
            noisy = cv2.resize(noisy, (self.crop_size, self.crop_size)); gt = cv2.resize(gt, (self.crop_size, self.crop_size)); h, w = self.crop_size, self.crop_size
...
for epoch in range(START_EPOCH, NUM_EPOCHS+1):
    ...
    for i, (noisy, gt) in enumerate(pbar):
        ...
        total_loss += criterion(pred, gt).item()
        pbar.set_postfix({'loss': total_loss/(i+1)})
    scheduler.step()
    avg_loss = total_loss / len(loader)
    print(f'Epoch {epoch} done, avg_loss={avg_loss:.6f}, lr={scheduler.get_last_lr()[0]:.2e}')
    torch.save(model.state_dict(), os.path.join(CKPT_DIR, f'restormer_ft_epoch{epoch}.pth'))
    torch.save(model.state_dict(), os.path.join(PROJECT_DIR, 'weights', 'restormer_ft_epoch{}.pth'.format(epoch)))

print('Done! Fine-tuned weights saved to checkpoints/ and weights/')
